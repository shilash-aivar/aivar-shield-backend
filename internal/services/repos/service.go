...
func (s *Service) Register(ctx context.Context, req models.RegisterRepoRequest) (models.Repo, error) {
	...
	row := s.pool.QueryRow(ctx, `
		INSERT INTO repos (id, org, name, full_name, repo_type, owner, tf_owner)
		...
		SET repo_type = EXCLUDED.repo_type,
		    ...
		    updated_at = now()
		RETURNING id, org, name, full_name, repo_type, owner, tf_owner, created_at, updated_at
	`, id, parts[0], parts[1], req.FullName, req.Type, nullString(req.Owner), nullString(req.TFOwner))

	var repo models.Repo
	var owner, tfOwner *string
	if err := row.Scan(
		&repo.ID, &repo.Org, &repo.Name, &repo.FullName, &repo.RepoType,
		&owner, &tfOwner, &repo.CreatedAt, &repo.UpdatedAt,
	); err != nil {
...
func (s *Service) GetByFullName(ctx context.Context, fullName string) (models.Repo, error) {
	row := s.pool.QueryRow(ctx, `
		SELECT id, org, name, full_name, repo_type, owner, tf_owner, created_at, updated_at
